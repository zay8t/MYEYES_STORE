  ...
  useEffect(() => {
    if (!initialProduct && slug) {
      fetch(`/api/products?slug=${encodeURIComponent(slug)}`, { cache: "no-store" })
        .then((r) => r.json())
        .then((data: ProductData[] | ProductData) => {
          if (Array.isArray(data)) {
            const found = data.find((p) => p.slug === slug) || data[0];
          ...
          <div className="lg:col-span-5 space-y-6">
            <div>
              <div className="flex items-center gap-2 mb-2 flex-wrap">
                ...
                <span className="text-xs text-slate-400 font-medium">
                  {product.gender} · {product.material ? product.material.replace("_", " ") : "ACETATE"}
                </span>
              </div>
            ...
            <div className="p-4 rounded-2xl bg-slate-50/80 border border-slate-100 space-y-3">
              ...
              <p className="text-[11px] text-slate-500 text-center font-medium">
                Standard Fit: 51-19-145mm · Frame Shape: {product.frameShape ? product.frameShape.replace("_", " ") : "CLASSIC"}
              </p>
            </div>
