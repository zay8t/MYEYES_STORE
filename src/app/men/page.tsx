...
function MenCatalogContent() {
  ...
  useEffect(() => {
    fetch("/api/products?gender=Men", { cache: "no-store" })
      .then((r) => r.json())
      .then((data) => {
        ...
        setProducts(menList.length > 0 ? menList : list);
        setLoading(false);
      })
      .catch(() => {
        setProducts([]);
  ...
  if (loading) {
    return <LogoLoader text="LOADING MEN'S COLLECTION..." />;
  }
...
export default function MenCollectionPage() {
  return (
    <Suspense fallback={<LogoLoader text="LOADING MEN'S COLLECTION..." />}>
      <MenCatalogContent />
    </Suspense>
  );
}
