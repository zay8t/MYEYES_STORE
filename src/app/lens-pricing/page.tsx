            ...
            {CORE_LENSES.slice(0, 3).map((lens) => (
              <div key={lens.id} className="bg-white border border-slate-200/70 hover:border-amber-300/60 rounded-3xl p-7 flex flex-col justify-between hover:shadow-xl hover:shadow-amber-500/5 transition-all duration-300">
                <div className="space-y-3">
                  <h3 className="text-slate-900 font-bold text-lg tracking-tight mb-1">
                    {lens.name}
                  </h3>
                  <div className="text-[11px] font-bold text-amber-700 bg-amber-50 border border-amber-100 rounded-lg px-2.5 py-1 inline-block mb-3">
                    Starting from Rs. {basePrices[lens.baseKey as keyof BasePriceConfig]}/-
                  </div>
                ...
                <div className="mt-8 pt-8 border-t border-slate-100">
                  {!calculationResult ? (
                    <div className="p-6 rounded-2xl bg-red-50 border border-red-200 flex items-start gap-3">
                      ...
                      <div>
                        <h4 className="text-sm font-bold text-red-900">Custom RX Required — Contact Support</h4>
                        ...
                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4">
                          <div>
                            <span className="text-sm font-semibold text-slate-700 block mb-1">
                              Prescription Lenses — +Rs. {calculationResult.finalPrice.toLocaleString()}/-
                            </span>
