...
export default async function AdminLensPricingPage() {
  ...
  try {
    ...
    if (count === 0) {
      await prisma.lensOption.createMany({
        data: SOLEX_LENS_OPTIONS.map(lens => ({
          ...
          isConfiguratorVisible: !["bifocal-round-top", "bifocal-flat-top", "progressive-freeform", "sv-159-pc", "sv-156-hmc"].includes(lens.id),
        })),
      });
    }
    ...
    lensOptions = await prisma.lensOption.findMany({
      ...
      select: { id: true, name: true, price: true, type: true, index: true, description: true, isConfiguratorVisible: true },
    });
  } catch (error) {
