    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-slate-200/80 rounded-2xl shadow-xs overflow-hidden">
        <div className="px-5 py-3.5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <h2 className="text-sm font-bold text-slate-900">MY EYES Core Lens Packages</h2>
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">5 ACTIVE LENSES</span>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/30">
                <th className="px-5 py-3 text-left font-bold text-slate-500 uppercase tracking-wider">Clean Name</th>
                <th className="px-4 py-3 text-left font-bold text-slate-500 uppercase tracking-wider">Base Code</th>
                <th className="px-4 py-3 text-left font-bold text-slate-700 uppercase tracking-wider">Standard Base Price</th>
                <th className="px-4 py-3 text-left font-bold text-amber-700 uppercase tracking-wider">Presbyopia (+40) Base Price</th>
                <th className="px-4 py-3 text-left font-bold text-slate-500 uppercase tracking-wider">Action</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {options.filter(l => CORE_FIVE_SET.has(l.id)).map(lens => {
                const isEditing = editingId === lens.id;
                const isSaved = savedId === lens.id;
                const baseCode = BASE_CODES[lens.id] || "N/A";
                return (
                    ...
                    <td className="px-4 py-3.5">
                      {isEditing ? (
                        ...
                        <button
                          ...
                          className="inline-flex items-center gap-1 text-[11px] font-bold text-slate-600 hover:text-amber-600 transition-colors cursor-pointer"
                        >
