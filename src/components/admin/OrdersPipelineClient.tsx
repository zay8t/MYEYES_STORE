...
import { updateOrderStatusAction, updatePaymentStatusAction, deleteOrderAction } from "@/app/actions/admin";
...
export default function OrdersPipelineClient({ initialOrders }: OrdersPipelineClientProps) {
  ...
  const [deletingOrder, setDeletingOrder] = useState<OrderReceiptData | null>(null);
  ...
  const handleConfirmDelete = async () => {
    ...
    const orderDisplayNum = deletingOrder.orderNumber
      ? String(deletingOrder.orderNumber).padStart(8, "0")
      : formatOrderNumber(deletingOrder || orderId);
    ...
    try {
      ...
      if (res.success) {
        ...
        setToast({ message: `Order #${orderDisplayNum} permanently deleted`, type: "success" });
                      ...
                      <td className="px-5 py-4 text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <button
                            ...
                            className="p-1.5 rounded-xl border border-rose-200 bg-white hover:bg-rose-50 text-rose-600 hover:text-rose-700 transition-colors cursor-pointer"
              ...
              >
                ...
                <div className="space-y-2 pt-2 border-t border-slate-100">
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap">
                    ...
                    <button
                      onClick={() => setDeletingOrder(order)}
                      className="p-2 rounded-xl border border-rose-200 bg-white hover:bg-rose-50 text-rose-600 hover:text-rose-700 transition-colors min-h-[38px] min-w-[38px] flex items-center justify-center cursor-pointer"
                      title="Delete Order Permanently"
                    >
                      <Trash2 className="w-4 h-4" />
                    </button>
          ...
          <div className="w-full max-w-md bg-white rounded-3xl p-6 shadow-2xl border border-slate-100 space-y-4">
            <div className="flex items-start gap-3.5">
              ...
              <div className="space-y-1">
                <h3 className="text-base font-bold text-slate-900">
                  Delete Order #{deletingOrder.orderNumber ? String(deletingOrder.orderNumber).padStart(8, "0") : formatOrderNumber(deletingOrder)}?
                </h3>
                <p className="text-xs text-slate-500 leading-relaxed">
                  Are you sure you want to permanently delete Order #{deletingOrder.orderNumber ? String(deletingOrder.orderNumber).padStart(8, "0") : formatOrderNumber(deletingOrder)} ({deletingOrder.customerName})?
                </p>
                <p className="text-xs font-semibold text-rose-600 bg-rose-50 border border-rose-100 rounded-xl px-3 py-2 mt-2">
                  This will remove the record from all sales figures and analytics.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-2.5 pt-2 border-t border-slate-100">
              ...
              <button
                ...
                disabled={isDeleting}
                className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold transition-colors cursor-pointer disabled:opacity-50 shadow-sm"
              >
                {isDeleting ? (
                  <>
